...
for (int j = 0; j < 3; j++) {
    ...
    if (j == 0) {
        ...
        CustomKeywords.'sample.upsellingMessages.getPlayerUpsellingMessagesMap'(packageConfigurationResBody, 'SHAHID_VIP_GEA', 
            'GEA')
    } else if (j == 1) {
        packageConfigurationRes = WS.sendRequest(findTestObject('Object Repository/Upselling Messages/packageConfiguration', 
    ...
    if (!((GlobalVariable.productMap['messageDisplayName']).equals(CustomKeywords.'sample.upsellingMessages.getData'(3, 
        ...
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                3, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(3, column), GlobalVariable.productMap[
            'messageDisplayName'])

        KeywordUtil.markFailed((('Title not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(3, column)) + 
    ...
    if (!((GlobalVariable.catalogMap['player_message_subscribe']).equals(CustomKeywords.'sample.upsellingMessages.getData'(
            5, column).toString().trim()))) {
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                5, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(5, column), GlobalVariable.catalogMap[
            'player_message_subscribe'])

        KeywordUtil.markFailed((('Text Anonymous User not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(
                5, column)) + 'Actual :') + (GlobalVariable.catalogMap['player_message_subscribe']))
    }
