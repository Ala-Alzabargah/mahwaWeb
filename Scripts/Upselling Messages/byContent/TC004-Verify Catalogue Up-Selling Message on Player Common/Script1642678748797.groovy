...
for (int j = 0; j < 3; j++) {
	...
	if (!((GlobalVariable.catalogMap['icon_1.label']).equals(CustomKeywords.'sample.upsellingMessages.getData'(6, column).toString().trim()))) {
		CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
				6, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(6, column), GlobalVariable.catalogMap[
			'icon_1.label'])
	...
	if (!((GlobalVariable.catalogMap['icon_2.label']).equals(CustomKeywords.'sample.upsellingMessages.getData'(7, column).toString().trim()))) {
		CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
				7, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(7, column), GlobalVariable.catalogMap[
			'icon_2.label'])

		KeywordUtil.markFailed((('watchOfflineIcons not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(
				7, column).toString().trim()) + 'Actual :') + (GlobalVariable.catalogMap['icon_2.label']).toString().trim())
	}
	...
	if (!((GlobalVariable.catalogMap['icon_3.label']).toString().trim().equals(CustomKeywords.'sample.upsellingMessages.getData'(
		...
		CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
				8, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(8, column), GlobalVariable.catalogMap[
			'icon_3.label'])

		KeywordUtil.markFailed((('watchOnTVIcons not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(
				8, column)) + 'Actual :') + (GlobalVariable.catalogMap['icon_3.label']).toString().trim())
	}
}