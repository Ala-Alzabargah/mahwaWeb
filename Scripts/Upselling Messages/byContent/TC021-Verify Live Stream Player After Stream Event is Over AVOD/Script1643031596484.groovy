...
for (int j = 0; j < 3; j++) {
    ...
    if (!((GlobalVariable.catalogMap['live_player_message_event_end']).toString().trim().equals(CustomKeywords.'sample.upsellingMessages.getData'(40, 
        ...
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                40, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(40, column), GlobalVariable.catalogMap[
            'live_player_message_event_end'])

        KeywordUtil.markFailed((('Title not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(40, column)) + 
            'Actual :') + (GlobalVariable.catalogMap['live_player_message_event_end']))
    }
