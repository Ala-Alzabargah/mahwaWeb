...
for (int j = 0; j < 3; j++) {
    ...
    if (!((GlobalVariable.catalogMap['icon_1.label']).equals(CustomKeywords.'sample.upsellingMessages.getData'(32, column).toString().trim()))) {
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                32, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(32, column), GlobalVariable.catalogMap[
            'icon_1.label'])
    ...
    if (!((GlobalVariable.catalogMap['icon_2.label']).equals(CustomKeywords.'sample.upsellingMessages.getData'(33, column).toString().trim()))) {
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                33, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(33, column), GlobalVariable.catalogMap[
            'icon_2.label'])

        KeywordUtil.markFailed((('watchOfflineIcons not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(
                33, column).toString().trim()) + 'Actual :') + (GlobalVariable.catalogMap['icon_2.label']).toString().trim())
    }
    ...
    if (!((GlobalVariable.catalogMap['icon_3.label']).toString().trim().equals(CustomKeywords.'sample.upsellingMessages.getData'(
        ...
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                34, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(34, column), GlobalVariable.catalogMap[
            'icon_3.label'])

        KeywordUtil.markFailed((('watchOnTVIcons not matche Expected :' + CustomKeywords.'sample.upsellingMessages.getData'(
                34, column)) + 'Actual :') + (GlobalVariable.catalogMap['icon_3.label']).toString().trim())
    }
}