...
for (int j = 0; j < 3; j++) {
    ...
    if (!((GlobalVariable.catalogMap['displayName']).equals(CustomKeywords.'sample.upsellingMessages.getData'(20, column).toString().trim()))) {
        CustomKeywords.'sample.upsellingMessages.updateUpsellResultSheet'(CustomKeywords.'sample.upsellingMessages.getData'(
                20, 'B'), 'byContent', CustomKeywords.'sample.upsellingMessages.getData'(20, column), GlobalVariable.catalogMap[
            'displayName'])
